"""
Bybit 取引所モジュール（既存 exchange.py からリファクタ）
...
class ExchangeBybit(ExchangeBase):
    ...
    def __init__(self):
        ...
        # paper モードは残高を模擬（USDT建て）
        self._paper_balance = {
            "USDT": config.INITIAL_CAPITAL_USDT,
            "BTC": 0.0,
        }

        if self.mode == "paper":
            logger.info(f"[Bybit/PAPER] 模擬取引モードで起動 (初期資金: {config.INITIAL_CAPITAL_USDT} USDT)")
        else:
            logger.warning("[Bybit/LIVE] 本番取引モードで起動 - 実際の資金が使われます！")
    ...
    def place_order(self, side: str, amount: float, price: float = None) -> dict:
        ...
        if self.mode == "paper":
            ...
            if side == "buy":
                if self._paper_balance["USDT"] < cost + fee:
                    logger.warning(
                        f"[Bybit/PAPER] 残高不足: 必要={cost + fee:.4f} USDT, "
                        f"残高={self._paper_balance['USDT']:.4f} USDT"
                    )
                    return {}
                ...
                if self._paper_balance["BTC"] < amount:
                    logger.warning(
                        f"[Bybit/PAPER] BTC残高不足: 必要={amount:.6f}, "
            ...
            logger.info(f"[Bybit/PAPER] {side.upper()} {amount:.6f} BTC @ {exec_price:.2f} USDT (手数料: {fee:.4f} USDT)")
            logger.info(f"[Bybit/PAPER] 残高: {self._paper_balance['USDT']:.4f} USDT, {self._paper_balance['BTC']:.6f} BTC")
        ...
        try:
            ...
            logger.info(f"[Bybit/LIVE] 注文完了: {side} {amount} BTC @ {price or 'market'}")
            return order
    ...
    def cancel_all_orders(self, symbol: str = config.SYMBOL) -> None:
        if self.mode == "paper":
            logger.info("[Bybit/PAPER] 全注文キャンセル（模擬）")
            return
        try:
